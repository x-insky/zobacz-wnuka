...
<body>
 ...
 <div id="glowna">
			<div>
			<h2>A poniżej może pojawią się zdjęcia wnuków, o ile podamy dobry adres i skrypt zadziała prawidłowo...</h2>
			</div>
	...
	<footer id="stopka">&copy;2017 v0.1b <button id="poco_button">Ale po co?</button> <button id="pomoc_button">Pomoc</button>
  ...
  <h3>Witryna wymaga dostępu do lokalizacji, skąd pochodzą oryginalne materiały.</h3>
 	...
 	<h3>Kopiuj-wklej</h3>
 	 <p>Bieżący serwis wymaga podania działającego odnośnika do serwisu zlobek.chojnow.eu, konkretnie do jednej z wielu galerii. Odnośnik z paska adresu należy wpisać (wkleić) w pole formularza. Prawdopodobnie ten mechanizm jest nieco utrudniony przy posługiowaniu się przeglądarką mobilną, korzytając z telefonu (tzw. smartfon). Dowolna przeglądarka www z komputera zapewnia machanizm kopiowania.  
 	<h3>Podstrony</h3>
...
$('#glowna div:first').hide(100);	
$('#skladowisko').show(100);	// pokaż kontener na zaczytaną zawartość
	
var $lista_podstron = $('a.link_tresc');

	if ( $lista_podstron.length >= 1 )	// pierwszy już zaczytano z load(), teraz ponowienie dla kolejnych podstron; 
